export const evolve_data: any = {
  challengeID: 12345,
  content: {
    battery: {
      currentValue: 76,
      growth: 13,
      amountOfDays: 15,
    },
    participants: {
      count: 127,
      gender: {
        male: 50,
        female: 77,
      },
      activeUsers: [
        {
          userName: "DashikMe",
          userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
        },
        {
          userName: "Apolog_Oleg7",
          userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
        },
        {
          userName: "Julia_1984",
          userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
        },
        {
          userName: "Andrew3",
          userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
        },
      ],
    },
    indicators: {
      engagementIndex: {
        value: 27,
        growth: 3,
      },
      activityIndex: {
        value: 21,
        growth: 4,
      },
      confidenceGrowthIndex: {
        value: 11,
        growth: 2,
      },
    },
    recommendations: [
      {
        title: "Снизить индекс выгорания",
        users: [
          {
            userName: "Лазарев Геваргиз",
            userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
            userRecomendations: ["лишняя нагрузка", "усталость на работе"],
          },
          {
            userName: "Anny",
            userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
            userRecomendations: ["выгорание", "усталость на работе"],
          },
        ],
      },
      {
        title: "Увеличить индекс командной работы",
        users: [
          {
            userName: "Andrew86",
            userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
            userRecomendations: ["низкая социализация"],
          },
          {
            userName: "Лазарев Геваргиз",
            userProfilePhotoURL: "/photo-1438761681033-6461ffad8d80.jfif",
            userRecomendations: ["лишняя нагрузка", "усталость на работе"],
          },
        ],
      },
    ],
    popularTasks: [
      {
        taskName: "Мой первый распорядок дня",
        taskDateBegin: "2022, 4, 18",
        taskDateEnd: "2022, 4, 20",
        membersCount: 46,
      },
      {
        taskName: "Командная работа",
        taskDateBegin: "2022, 4, 19",
        taskDateEnd: "2022, 4, 22",
        membersCount: 34,
      },
    ],
    pulseSurveysResult: [
      {
        surveyID: 1,
        results: {
          conclusionFirst: {
            text: "Не слышит руководство",
            percent: 23,
          },
          conclusionSecond: {
            text: "Отсутствует мотивация к работе",
            percent: 37,
          },
          conclusionThird: {
            text: "Проблемы в семье мешают на работе",
            percent: 19,
          },
          conclusionFourth: {
            text: "Тяжело работать в текущей команде",
            percent: 21,
          },
        },
      },
    ],
    visitsCount: [
      {
        date: "2022, 5, 22",
        value: 45,
      },
      {
        date: "2022, 5, 23",
        value: 30,
      },
      {
        date: "2022, 5, 24",
        value: 60,
      },
      {
        date: "2022, 5, 25",
        value: 45,
      },
      {
        date: "2022, 5, 26",
        value: 35,
      },
      {
        date: "2022, 5, 27",
        value: 25,
      },
      {
        date: "2022, 5, 28",
        value: 55,
      },
      {
        date: "2022, 5, 29",
        value: 25,
      },
      {
        date: "2022, 5, 30",
        value: 55,
      },
      {
        date: "2022, 5, 1",
        value: 25,
      },
      {
        date: "2022, 5, 2",
        value: 30,
      },
    ],
  },
};
